...
追加セクション:
- 制約事項（外部依存管理、仕様と実装の整合性、データ永続化）
- ガバナンス（開発ワークフロー、ブランチ戦略、レビューと承認、開発方針、憲法の改訂、テンプレートとの整合性）
- 品質保証（トークン効率と完全性、文字エンコーディング、テンプレートのローカライゼーション）

テンプレート更新状況:
- ✅ .specify/templates/spec-template.md - 確認済み（Constitution Check項目が存在）
- ✅ .specify/templates/plan-template.md - 確認済み（Constitution Check項目が存在）
- ✅ .specify/templates/tasks-template.md - 確認済み（テストファースト言及あり）
- ⚠ .specify/templates/checklist-template.md - 未確認（次回更新で整合性チェック）
...
flowchart TB
    ...
    subgraph "仕様ブランチ"
        SPEC[001-todo-app<br/>仕様定義]
    end
    
    subgraph "実装ブランチ"
        IMPL[feature/impl-001-todo-app<br/>機能実装]
    end
...
**命名規則**:
- 仕様ブランチ: `<番号>-<短い名前>` （例: `001-todo-app`）
- 実装ブランチ: `feature/impl-<番号>-<短い名前>` （例: `feature/impl-001-user-page`）
...
**バージョン**: 1.0.0 | **制定日**: 2025-11-14 | **最終改訂日**: 2025-11-14
