...
**重大変更の定義**:以下の変更は、必ずレビュー承認を得なければならない（MUST）:
...
- Python 3.10.11を標準実行環境とする（MUST）
- Pythonコマンド実行時は `py -3.10`を使用しなければならない（MUST）
- `python`コマンドは使用してはならない（MUST NOT）

**ビルドとプレビュー検証**:

- GitHub Pagesデプロイ前にローカルでビルドとプレビューを検証しなければならない（MUST）
- テストが100%正常に機能が動作するまで繰り返し修正しなければならない（MUST）
- 機能が正常に動作しない状態でデプロイしてはならない（MUST NOT）

**根拠**:
本番デプロイ前のローカル検証は、デプロイ後の問題を防ぎ、開発効率を向上させる。特定のPythonバージョンを使用することで、環境の再現性を確保する。
...
- E2Eテストが100%正常に機能するまで繰り返し修正しなければならない（MUST）
...
4. **ガントチャート要件**:
   ...
   - 土日（weekends）と年末年始休日（12/27-1/4）を除外しなければならない（MUST）
   - 相対日付方式（`after p<N>`）を使用し、開始日変更で全体が自動調整されるようにしなければならない（MUST）
   - 休日に着色しスケジュールを入れてはならない（MUST NOT）
   - 各フェーズの依存関係を明確に定義しなければならない（MUST）
...
- 一括置換は文字化けの原因となるため、使用してはならない（MUST NOT）
...
- ドキュメント内のリンクは、ローカルパスではなくGitHubリポジトリURLを使用しなければならない（MUST）
- リポジトリURL: `https://github.com/J1921604/ToDo`を標準とする（MUST）
- すべてのドキュメントリンクは正確性を検証しなければならない（MUST）

**根拠**:
GitHubリポジトリURLを使用することで、ドキュメントがどこから参照されても正しくリンクが機能し、共有や公開が容易になる。

### バージョン管理

**原則**:

- プロジェクトのバージョンは1.0.0で統一しなければならない（MUST）
- 作成日や更新日は2025年11月20日で統一しなければならない（MUST）
- バージョン番号の不整合があってはならない（MUST NOT）

**根拠**:
バージョンと日付の統一は、ドキュメント間の整合性を保ち、混乱を防ぐ。
...
**Version**: 1.0.0 | **Ratified**: 2025-11-20
