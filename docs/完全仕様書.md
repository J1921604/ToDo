...
stateDiagram-v2
    [*] --> タスク入力
    タスク入力 --> バリデーション: Enter/追加ボタン
    バリデーション --> エラー表示: 空文字/500文字超過
    エラー表示 --> タスク入力
    バリデーション --> タスク生成: 1-500文字
    タスク生成 --> State更新: createTodoItem()
    State更新 --> LocalStorage保存: saveTodos()
    LocalStorage保存 --> UI更新: 再レンダリング
    UI更新 --> [*]
    
    note right of バリデーション
        trim()で前後空白除去
        長さチェック: 1-500文字
    end note
...
```mermaid
flowchart TD
    Start[チェックボックスクリック] --> Find[タスク検索]
    Find --> Toggle[completed反転]
    Toggle --> Update[State更新]
    Update --> Save[LocalStorage保存]
    Save --> Render[UI更新]
    Render --> End[完了]
    
    style Start fill:#e3f2fd
    style Toggle fill:#fff3e0
    style Save fill:#f3e1ff
